...
class Parser:
    ...
    def _init_commands(self, fileName):
        ...
        for line in commands:
            ...
            temp_line = temp_line.strip("\r")  # remove carriage_return
    ...
    def _parseCurrCommand(self):
        ...
        if len(command_parts) == Consts.C_OPERATOR and command_parts[
            0] in Consts.C_AR_LIST:
            self.__commandType = Consts.C_ARITHMETIC
            self.__arg1 = command_parts[0]  # arithmetic - keep command itself
            return
        elif len(command_parts) == Consts.C_CMPLX and \
                        command_parts[0] in (
                    Consts.C_P_LIST + Consts.C_FUNC_LIST):
            self.__commandType = command_parts[0]
            self.__arg1 = command_parts[1]
            self.__arg2 = int(command_parts[2])
            return
        elif len(command_parts) == Consts.C_PFLOW and \
                        command_parts[0] in Consts.C_PFLOW_LIST:
            self.__commandType = command_parts[0]
            self.__arg1 = command_parts[1]
            return
        elif command_parts[0] == Consts.C_RETURN:
            self.__commandType = command_parts[0]
        else:
